...
ARCHETYPES = {
    ...
    "Stretch Big":   dict(cost=3, spacing=2, behavior="spot_up", abbr="C",
    ...
    # --- new archetypes (2026-06-22 guard-tier expansion) ---
    "Microwave Scorer": dict(cost=3, spacing=1, behavior="pull_up", abbr="SG",
                          base=dict(off=7, dfn=3, pas=4, stl=4)),
    "Three-Level Scorer": dict(cost=3, spacing=1, behavior="pull_up", abbr="SG",
                          base=dict(off=7, dfn=4, pas=4, stl=4)),
    "Downhill Guard": dict(cost=3, spacing=1, behavior="cut", abbr="PG",
                          base=dict(off=6, dfn=4, pas=5, stl=5)),
    "Utility Wing":  dict(cost=3, spacing=1, behavior="cut", abbr="WG",
                          base=dict(off=4, dfn=6, pas=6, stl=5)),
    "Playmaking Big": dict(cost=3, spacing=1, behavior="distribute", abbr="C",
                          base=dict(off=5, dfn=6, pas=7, stl=3)),
    "Energy Big":    dict(cost=3, spacing=0, behavior="roll", abbr="C",
                          base=dict(off=5, dfn=7, pas=2, stl=3)),
}
...
ARCHETYPE_POSITIONS = {
    ...
    "Post Scorer":   [4, 5],
    "Defensive Anchor": [5],
    "Microwave Scorer": [1, 2],
    "Three-Level Scorer": [2, 3],
    "Downhill Guard": [1, 2],
    "Utility Wing":  [2, 3, 4],
    "Playmaking Big": [4, 5],
    "Energy Big":    [4, 5],
...
ARCHETYPE_ANCHORS = {
    ...
    "Microwave Scorer": [(60, 75), (47, 38)],
    "Three-Level Scorer": [(55, 70), (47, 33)],
    "Downhill Guard": [(45, 22), (50, 40)],
    "Utility Wing":  [(80, 40), (60, 20)],
    "Playmaking Big": [(50, 42), (55, 20)],
    "Energy Big":    [(53, 13), (51, 39)],
}
...
ARCHETYPE_ZONE_MIX = {   # (rim, midrange, three)
    ...
    "Microwave Scorer": (0.08, 0.34, 0.58),
    "Three-Level Scorer": (0.13, 0.37, 0.50),
    "Downhill Guard":   (0.22, 0.30, 0.48),
    "Utility Wing":     (0.14, 0.25, 0.61),
    "Playmaking Big":   (0.30, 0.40, 0.30),
    "Energy Big":       (0.50, 0.35, 0.15),
}
...
ARCHETYPE_TENDENCY = {
    ...
    "Stretch Big":     {"handle": 2,  "shoot": 1.10, "pass": 3},
    ...
    "Microwave Scorer": {"handle": 6, "shoot": 1.40, "pass": 4},
    "Three-Level Scorer": {"handle": 6, "shoot": 1.35, "pass": 5},
    "Downhill Guard":  {"handle": 8,  "shoot": 1.20, "pass": 6},
    "Utility Wing":    {"handle": 4,  "shoot": 0.85, "pass": 8},
    "Playmaking Big":  {"handle": 4,  "shoot": 0.90, "pass": 11},
    "Energy Big":      {"handle": 1,  "shoot": 0.90, "pass": 2},
}
...
ARCHETYPE_REB = {
    ...
    "Two-Way Forward": 6, "Stretch Big": 7,
    "Rim-Run Big": 8, "Post Scorer": 8, "Screening Big": 9, "Defensive Anchor": 9,
    "Microwave Scorer": 3, "Three-Level Scorer": 4, "Downhill Guard": 3,
    "Utility Wing": 5, "Playmaking Big": 8, "Energy Big": 9,
}
...
SHOT_PROFILE = {
    ...
    "Microwave Scorer": (0.4, 0.85, 0.9),
    "Three-Level Scorer": (0.7, 0.9, 0.75),
    "Downhill Guard":  (1.0, 0.5, 0.4),
    "Utility Wing":    (0.5, 0.5, 0.7),
    "Playmaking Big":  (0.7, 0.6, 0.5),
    "Energy Big":      (1.0, 0.2, 0.0),
}
...
ARCHETYPE_DEF_SPLIT = {   # (on_ball, block, steal)
    ...
    "Stretch Big":     (0.5, 0.6, 0.35), "Screening Big": (0.4, 0.9, 0.2),
    ...
    "Two-Way Forward": (0.8, 0.4, 0.7),
    "Post Scorer":     (0.4, 0.8, 0.2), "Defensive Anchor": (0.5, 1.0, 0.3),
    "Microwave Scorer": (0.6, 0.1, 0.5), "Three-Level Scorer": (0.7, 0.2, 0.5),
    "Downhill Guard":  (0.7, 0.2, 0.6), "Utility Wing": (0.7, 0.3, 0.6),
    "Playmaking Big":  (0.5, 0.6, 0.3), "Energy Big": (0.4, 0.9, 0.2),
}
...
CHEMISTRY = [
    ...
    dict(name="Splash Brothers", kind="duo",
         archetypes=["Shooting SF", "Stretch Big"],
    ...
    dict(name="Sniper Corps", kind="duo",
         archetypes=["Sharpshooter", "Stretch Big"],
...
_POOL_RAW = [
    # --- Tier 1 ---
    ('Aaron Harris', 'Scoring PG', 1, 8, 4, 5, 2, 'iso_threat'),
    ('Davon Ross', 'Scoring PG', 1, 7, 7, 6, 4, 'turnover_prone'),
    ('Josh Cunningham', 'Scoring PG', 1, 8, 6, 6, 2, 'turnover_prone'),
    ('Elan Diallo', 'Slashing SF', 1, 3, 7, 3, 5, 'iso_threat'),
    ('Isaiah Tate', 'Slashing SF', 1, 3, 8, 4, 5, 'soft_touch'),
    ('Xavier Fox', 'Slashing SF', 1, 3, 8, 4, 6, 'soft_touch'),
    ('Garrett Payne', 'Shooting SF', 1, 8, 6, 4, 2, 'catch_shoot'),
    ('Silas Barnes', 'Shooting SF', 1, 8, 8, 2, 3, 'catch_shoot'),
    ('Trevor Turner', 'Shooting SF', 1, 8, 8, 3, 2, 'catch_shoot'),
    ('Avery Diallo', 'Rim-Run Big', 1, 2, 8, 3, 8, 'soft_touch'),
    ('Chase Cruz', 'Rim-Run Big', 1, 2, 8, 2, 6, 'putback'),
    ('Ezra Phillips', 'Rim-Run Big', 1, 2, 8, 3, 8, 'lob_threat'),
    ('Logan Peters', 'Pass-First PG', 1, 2, 8, 8, 2, 'floor_general'),
    ('Mike Fletcher', 'Pass-First PG', 1, 3, 6, 8, 2, 'tempo_control'),
    ('Sol Mensah', 'Pass-First PG', 1, 2, 8, 8, 2, 'tempo_control'),
    ('Trent Hall', 'Pass-First PG', 1, 2, 8, 7, 2, 'tempo_control'),
    ('Jerry Mitchell', 'Lockdown Wing', 1, 3, 8, 2, 2, 'pickpocket'),
    ('Nico Hughes', 'Lockdown Wing', 1, 2, 8, 4, 2, 'on_ball_menace'),
    ('Tyrese Adams', 'Lockdown Wing', 1, 3, 8, 2, 2, 'on_ball_menace'),
    ('Austin Underwood', '3&D Wing', 1, 4, 8, 3, 2, 'catch_shoot'),
    ('Jeremiah Hill', '3&D Wing', 1, 4, 8, 2, 2, 'catch_shoot'),
    ('Mark Bradley', '3&D Wing', 1, 6, 8, 2, 2, 'on_ball_menace'),
    ('Emmanuel Glenn', 'Stretch Big', 1, 7, 8, 4, 3, 'soft_touch'),
    ('Grady Freeman', 'Stretch Big', 1, 8, 8, 3, 2, 'rim_protector'),
    ('Jaylon Graham', 'Stretch Big', 1, 6, 8, 4, 2, 'soft_touch'),
    ('Cam Woods', 'Point Forward', 1, 4, 5, 6, 2, 'tempo_control'),
    ('Jack Fox', 'Point Forward', 1, 4, 7, 6, 2, 'floor_general'),
    ('Micah Gibson', 'Point Forward', 1, 5, 5, 7, 2, 'tempo_control'),
    ('Damian Flowers', 'Screening Big', 1, 2, 8, 3, 4, 'ball_stopper'),
    ('Damian Gordon', 'Screening Big', 1, 2, 8, 3, 3, 'on_ball_menace'),
    ('Jaylon Anderson', 'Screening Big', 1, 2, 8, 3, 2, 'on_ball_menace'),
    ('Jackson Rhodes', 'Combo Guard', 1, 6, 6, 4, 2, 'deadeye'),
    ('Jaylen Cook', 'Combo Guard', 1, 8, 7, 5, 2, 'quick_release'),
    ('Ronnie Ross', 'Combo Guard', 1, 5, 5, 4, 3, 'handles'),
    ('Lamar Richardson', 'Sharpshooter', 1, 7, 6, 4, 2, 'catch_shoot'),
    ('Marcus Warner', 'Sharpshooter', 1, 8, 6, 4, 2, 'deadeye'),
    ('Mario Jenkins', 'Sharpshooter', 1, 8, 7, 2, 2, 'catch_shoot'),
    ('Quincy Taylor', 'Two-Way Forward', 1, 3, 8, 4, 5, 'soft_touch'),
    ('Sam Crawford', 'Two-Way Forward', 1, 2, 8, 4, 5, 'on_ball_menace'),
    ('Brock Griffin', 'Post Scorer', 1, 4, 7, 4, 7, 'iso_threat'),
    ('Zane Anderson', 'Post Scorer', 1, 4, 6, 4, 6, 'putback'),
    ('Miles Caldwell', 'Defensive Anchor', 1, 2, 8, 4, 2, 'rim_protector'),
    ('Ronnie Holmes', 'Defensive Anchor', 1, 2, 8, 2, 4, 'on_ball_menace'),
    ('Cole Murphy', 'Microwave Scorer', 1, 8, 5, 4, 3, 'heat_check'),
    ('Hollis Reed', 'Microwave Scorer', 1, 8, 4, 5, 4, 'streaky'),
    ('Gabe Porter', 'Three-Level Scorer', 1, 8, 4, 3, 3, 'soft_touch'),
    ('Ryan Robinson', 'Three-Level Scorer', 1, 8, 3, 3, 5, 'iso_threat'),
    ('Ivan Patterson', 'Downhill Guard', 1, 4, 3, 4, 5, 'turnover_prone'),
    ('Jalen Cunningham', 'Downhill Guard', 1, 5, 3, 6, 7, 'handles'),
    ('Javon Jones', 'Utility Wing', 1, 3, 6, 5, 2, 'tempo_control'),
    ('Jordan Sefu', 'Utility Wing', 1, 6, 7, 7, 3, 'floor_general'),
    ('Deon Green', 'Playmaking Big', 1, 3, 6, 6, 3, 'handles'),
    ('Tariq Freeman', 'Playmaking Big', 1, 4, 6, 7, 2, 'floor_general'),
    ('Jax Novak', 'Energy Big', 1, 2, 7, 2, 8, 'ball_stopper'),
    ('Wes Lindqvist', 'Energy Big', 1, 2, 8, 2, 8, 'lob_threat'),
    # --- Tier 2 ---
    ('Donovan Jones', 'Scoring PG', 2, 10, 6, 7, 2, 'iso_threat'),
    ('Tyler Underwood', 'Scoring PG', 2, 10, 7, 6, 4, 'handles'),
    ('Marcus Tran', 'Slashing SF', 2, 4, 10, 3, 6, 'eurostep'),
    ('Travis Diallo', 'Slashing SF', 2, 3, 8, 2, 6, 'iso_threat'),
    ('Ezra Lane', 'Shooting SF', 2, 10, 9, 4, 2, 'quick_release'),
    ('Trevor Allen', 'Shooting SF', 2, 11, 10, 5, 2, 'quick_release'),
    ('Ronnie Riley', 'Rim-Run Big', 2, 2, 9, 2, 8, 'ball_stopper'),
    ('Tobias Sullivan', 'Rim-Run Big', 2, 2, 11, 2, 9, 'soft_touch'),
    ('Knox Griffin', 'Pass-First PG', 2, 3, 11, 11, 3, 'dimer'),
    ('Miles Anderson', 'Pass-First PG', 2, 3, 10, 11, 3, 'handles'),
    ('Khalil Wallace', 'Lockdown Wing', 2, 3, 11, 5, 2, 'on_ball_menace'),
    ('Knox Bradley', 'Lockdown Wing', 2, 3, 11, 5, 2, 'lockdown'),
    ('Juju Riley', '3&D Wing', 2, 7, 11, 5, 2, 'corner_spec'),
    ('Noah Wright', '3&D Wing', 2, 8, 11, 4, 3, 'corner_spec'),
    ('Derrick Kringle', 'Stretch Big', 2, 10, 10, 3, 3, 'soft_touch'),
    ('Travis Dixon', 'Stretch Big', 2, 9, 9, 5, 2, 'soft_touch'),
    ('Jamal Wallace', 'Point Forward', 2, 6, 8, 10, 3, 'floor_general'),
    ('Tobias Locke', 'Point Forward', 2, 7, 9, 9, 2, 'tempo_control'),
    ('Dawson Ward', 'Screening Big', 2, 2, 11, 4, 4, 'on_ball_menace'),
    ('Jerry Peters', 'Screening Big', 2, 2, 11, 4, 6, 'shot_blocker'),
    ('Dawson Doyle', 'Combo Guard', 2, 8, 7, 8, 4, 'streaky'),
    ('Isaac Marsh', 'Combo Guard', 2, 7, 7, 7, 3, 'handles'),
    ('Carson Gordon', 'Sharpshooter', 2, 11, 9, 3, 2, 'limitless'),
    ('Myles Ross', 'Sharpshooter', 2, 11, 7, 3, 2, 'quick_release'),
    ('Micah Hamilton', 'Two-Way Forward', 2, 4, 11, 3, 6, 'on_ball_menace'),
    ('Zane Wilson', 'Two-Way Forward', 2, 3, 11, 4, 5, 'on_ball_menace'),
    ('Damon Phillips', 'Post Scorer', 2, 5, 9, 3, 10, 'iso_threat'),
    ('Landon Richardson', 'Post Scorer', 2, 4, 9, 3, 6, 'putback'),
    ('Micah Webb', 'Defensive Anchor', 2, 2, 11, 3, 3, 'shot_blocker'),
    ('Rell Murray', 'Defensive Anchor', 2, 2, 11, 4, 4, 'rim_protector'),
    ('Devin Udeze', 'Microwave Scorer', 2, 11, 4, 5, 4, 'iso_threat'),
    ('Kareem Lane', 'Microwave Scorer', 2, 11, 5, 4, 2, 'quick_release'),
    ('Ben Freeman', 'Three-Level Scorer', 2, 11, 4, 5, 6, 'deadeye'),
    ('Sol Coleman', 'Three-Level Scorer', 2, 10, 4, 3, 3, 'iso_threat'),
    ('Dillon Watson', 'Downhill Guard', 2, 5, 4, 7, 8, 'iso_threat'),
    ('Quincy Coleman', 'Downhill Guard', 2, 3, 4, 7, 7, 'killer_cross'),
    ('Avery Pavlov', 'Utility Wing', 2, 7, 10, 8, 4, 'on_ball_menace'),
    ('Mateo Greer', 'Utility Wing', 2, 7, 9, 7, 2, 'tempo_control'),
    ('Logan Vance', 'Playmaking Big', 2, 6, 9, 9, 2, 'floor_general'),
    ('Bradley Brown', 'Energy Big', 2, 2, 8, 2, 11, 'putback'),
    # --- Tier 3 ---
    ('Damon Washington', 'Scoring PG', 3, 12, 7, 9, 3, 'heat_check'),
    ('Ty Vance', 'Scoring PG', 3, 10, 8, 7, 3, 'turnover_prone'),
    ('Knox Pavlov', 'Slashing SF', 3, 4, 11, 3, 8, 'turnover_prone'),
    ('Myles Kemp', 'Slashing SF', 3, 5, 10, 5, 8, 'iso_threat'),
    ('Devin Doyle', 'Shooting SF', 3, 12, 11, 5, 2, 'corner_spec'),
    ('Rashad Scott', 'Shooting SF', 3, 12, 10, 4, 2, 'quick_release'),
    ('Isaac Taylor', 'Rim-Run Big', 3, 2, 12, 2, 10, 'ball_stopper'),
    ('Marquise Collins', 'Rim-Run Big', 3, 2, 12, 3, 12, 'soft_touch'),
    ('Miles Sanders', 'Pass-First PG', 3, 4, 11, 12, 2, 'floor_general'),
    ('Tyrell Holmes', 'Pass-First PG', 3, 4, 12, 12, 3, 'floor_general'),
    ('Mike Glenn', 'Lockdown Wing', 3, 5, 12, 5, 2, 'on_ball_menace'),
    ('Trey Fisher', 'Lockdown Wing', 3, 5, 12, 4, 2, 'on_ball_menace'),
    ('Tyler Cunningham', '3&D Wing', 3, 9, 12, 4, 3, 'catch_shoot'),
    ('Yusuf West', '3&D Wing', 3, 7, 12, 4, 2, 'deadeye'),
    ('Christian Payne', 'Stretch Big', 3, 11, 11, 3, 2, 'deadeye'),
    ('Derrick Ferguson', 'Stretch Big', 3, 10, 11, 4, 2, 'rim_protector'),
    ('Damon Cox', 'Point Forward', 3, 6, 9, 11, 3, 'handles'),
    ('Malik Ford', 'Point Forward', 3, 6, 9, 9, 5, 'dimer'),
    ('Isaac Glenn', 'Screening Big', 3, 2, 12, 7, 6, 'on_ball_menace'),
    ('Sol Lewis', 'Screening Big', 3, 2, 12, 5, 5, 'rim_protector'),
    ('Garrett Woods', 'Combo Guard', 3, 10, 7, 8, 2, 'iso_threat'),
    ('Omar Evans', 'Combo Guard', 3, 9, 7, 6, 3, 'deadeye'),
    ('Julian Vance', 'Sharpshooter', 3, 12, 10, 5, 2, 'limitless'),
    ('Quincy Webb', 'Sharpshooter', 3, 12, 11, 5, 2, 'corner_spec'),
    ('Brock Perry', 'Two-Way Forward', 3, 6, 12, 6, 7, 'pickpocket'),
    ('Jax Sefu', 'Two-Way Forward', 3, 3, 12, 5, 6, 'pickpocket'),
    ('Dwayne Rice', 'Post Scorer', 3, 6, 10, 6, 10, 'soft_touch'),
    ('Kyle Cruz', 'Post Scorer', 3, 6, 10, 3, 10, 'ball_stopper'),
    ('Anthony Hall', 'Defensive Anchor', 3, 2, 12, 5, 3, 'shot_blocker'),
    ('Jose Haas', 'Microwave Scorer', 3, 12, 5, 5, 3, 'heat_check'),
    ('Quincy Collins', 'Three-Level Scorer', 3, 12, 5, 3, 4, 'iso_threat'),
    ('Shane Murray', 'Downhill Guard', 3, 5, 6, 9, 11, 'eurostep'),
    ('Jackson Cook', 'Utility Wing', 3, 6, 11, 10, 3, 'dimer'),
    ('Trey Griffin', 'Playmaking Big', 3, 5, 9, 9, 4, 'floor_general'),
    ('Mario Cox', 'Energy Big', 3, 3, 12, 2, 12, 'putback'),
    # --- Tier 4 ---
    ('Dalton Hamilton', 'Scoring PG', 4, 13, 9, 9, 3, 'deadeye'),
    ('Dee Porter', 'Scoring PG', 4, 14, 7, 8, 4, 'fan_favorite'),
    ('Cody Locke', 'Slashing SF', 4, 6, 13, 5, 10, 'eurostep'),
    ('Justin Pike', 'Slashing SF', 4, 7, 14, 4, 9, 'eurostep'),
    ('Isaiah Peters', 'Shooting SF', 4, 15, 12, 4, 2, 'quick_release'),
    ('Solomon Bennett', 'Shooting SF', 4, 15, 14, 4, 3, 'catch_shoot'),
    ('Cameron Nelson', 'Rim-Run Big', 4, 4, 15, 4, 12, 'putback'),
    ('Jordan Williams', 'Rim-Run Big', 4, 3, 15, 5, 13, 'lob_threat'),
    ('Emmanuel Harris', 'Pass-First PG', 4, 4, 14, 14, 2, 'on_ball_menace'),
    ('Isaac West', 'Lockdown Wing', 4, 4, 15, 6, 2, 'interceptor'),
    ('Elan Phillips', '3&D Wing', 4, 8, 15, 6, 3, 'on_ball_menace'),
    ('Jax Love', 'Stretch Big', 4, 14, 12, 6, 4, 'deadeye'),
    ('Cam Carter', 'Point Forward', 4, 7, 10, 12, 4, 'iso_threat'),
    ('Ivan Baker', 'Screening Big', 4, 2, 15, 7, 7, 'on_ball_menace'),
    ('Justin Scott', 'Combo Guard', 4, 13, 12, 8, 5, 'iso_threat'),
    ('Davon White', 'Sharpshooter', 4, 15, 12, 5, 2, 'quick_release'),
    ('Will Bennett', 'Two-Way Forward', 4, 6, 15, 7, 8, 'on_ball_menace'),
    ('Reggie Watson', 'Post Scorer', 4, 7, 14, 5, 12, 'putback'),
    ('Zach Sullivan', 'Defensive Anchor', 4, 2, 15, 6, 4, 'on_ball_menace'),
    ('Greg Rhodes', 'Microwave Scorer', 4, 15, 7, 7, 4, 'quick_release'),
    ('Garrett Frazier', 'Three-Level Scorer', 4, 15, 8, 7, 7, 'soft_touch'),
    ('Dee Tran', 'Downhill Guard', 4, 6, 7, 10, 12, 'killer_cross'),
    ('Damian Franklin', 'Utility Wing', 4, 7, 13, 9, 4, 'floor_general'),
    ('Sam Ward', 'Playmaking Big', 4, 6, 12, 11, 3, 'floor_general'),
    ('Travis Voss', 'Energy Big', 4, 3, 14, 3, 15, 'rim_protector'),
    # --- Tier 5 ---
    ('Nash Morgan', 'Scoring PG', 5, 14, 11, 11, 6, 'handles'),
    ('Hunter Mills', 'Slashing SF', 5, 7, 15, 6, 11, 'killer_cross'),
    ('Rell Voss', 'Shooting SF', 5, 16, 13, 6, 4, 'catch_shoot'),
    ('Nick Kringle', 'Rim-Run Big', 5, 3, 16, 5, 14, 'dream_shake'),
    ('Tyrell Owens', 'Pass-First PG', 5, 6, 16, 16, 2, 'handles'),
    ('Finn Locke', 'Lockdown Wing', 5, 5, 16, 7, 2, 'on_ball_menace'),
    ('Corey Brooks', '3&D Wing', 5, 10, 16, 6, 2, 'quick_release'),
    ('Mark Price', 'Stretch Big', 5, 16, 15, 6, 5, 'rim_protector'),
    ('Ronnie Wallace', 'Point Forward', 5, 10, 11, 15, 4, 'dimer'),
    ('Kobi Vance', 'Screening Big', 5, 2, 16, 6, 8, 'rim_protector'),
    ('Flynn Rhodes', 'Combo Guard', 5, 13, 13, 10, 5, 'turnover_prone'),
    ('Cam Perry', 'Sharpshooter', 5, 16, 13, 7, 2, 'catch_shoot'),
    ('Bryce Mitchell', 'Two-Way Forward', 5, 6, 16, 7, 8, 'on_ball_menace'),
    ('Grant Baker', 'Post Scorer', 5, 7, 12, 7, 13, 'ball_stopper'),
    ('Quentin Hamilton', 'Defensive Anchor', 5, 2, 16, 7, 6, 'rim_protector'),
    ('Noah Miller', 'Microwave Scorer', 5, 16, 6, 8, 4, 'iso_threat'),
    ('Gabe Murphy', 'Three-Level Scorer', 5, 16, 8, 7, 6, 'heat_check'),
    ('Trent White', 'Downhill Guard', 5, 6, 9, 11, 13, 'killer_cross'),
    ('Jax Thompson', 'Utility Wing', 5, 10, 13, 11, 5, 'on_ball_menace'),
    ('Ben Fisher', 'Playmaking Big', 5, 7, 15, 14, 5, 'tempo_control'),
    # --- Tier 6 ---
    ('Myles Hughes', 'Scoring PG', 6, 16, 11, 12, 6, 'iso_threat'),
    ('Jackson West', 'Slashing SF', 6, 8, 17, 6, 15, 'iso_threat'),
    ('Kai Bennett', 'Shooting SF', 6, 20, 15, 7, 4, 'deadeye'),
    ('Finn Moore', 'Rim-Run Big', 6, 3, 20, 5, 16, 'rim_protector'),
    ('Amari Owens', 'Pass-First PG', 6, 7, 17, 18, 3, 'mentor'),
    ('Finn Clark', 'Lockdown Wing', 6, 4, 20, 7, 2, 'on_ball_menace'),
    ('Henry Lamonet', 'Lockdown Wing', 6, 5, 20, 6, 2, 'lockdown'),
    ('Yusuf Murray', '3&D Wing', 6, 12, 20, 5, 3, 'catch_shoot'),
    ('Cody Mraz', 'Stretch Big', 6, 16, 17, 6, 5, 'rim_protector'),
    ('Harrison Perry', 'Point Forward', 6, 10, 13, 16, 4, 'dimer'),
    ('Jabari Jones', 'Screening Big', 6, 2, 20, 8, 8, 'on_ball_menace'),
    ('Derrick Frost', 'Combo Guard', 6, 15, 12, 12, 5, 'quick_release'),
    ('Mason Adams', 'Sharpshooter', 6, 18, 15, 5, 3, 'quick_release'),
    ('Curtis Kringle', 'Two-Way Forward', 6, 7, 20, 7, 9, 'eurostep'),
    ('Marcus Rice', 'Post Scorer', 6, 9, 16, 6, 13, 'rim_protector'),
    # --- Tier 7 ---
    ('Jacoby Wallace', 'Scoring PG', 7, 18, 13, 11, 6, 'soft_touch'),
    ('Kareem Yates', 'Slashing SF', 7, 8, 19, 7, 15, 'iso_threat'),
    ('Travis Ortega', 'Shooting SF', 7, 21, 19, 9, 3, 'catch_shoot'),
    ('Cam Simmons', 'Rim-Run Big', 7, 4, 21, 5, 18, 'rim_protector'),
    ('Keon Lane', 'Pass-First PG', 7, 5, 20, 22, 2, 'handles'),
    ('Dillon Clark', 'Lockdown Wing', 7, 5, 25, 6, 2, 'pickpocket'),
    ('Quinn Porter', '3&D Wing', 7, 13, 25, 6, 5, 'catch_shoot'),
    ('Corey Rice', 'Stretch Big', 7, 18, 20, 8, 4, 'catch_shoot'),
    ('Dwayne Coleman', 'Point Forward', 7, 10, 16, 16, 7, 'dimer'),
    ('Logan Woods', 'Screening Big', 7, 3, 25, 8, 10, 'dream_shake'),
]
